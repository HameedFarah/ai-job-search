#!/usr/bin/env python3
"""ChatGPT discovery wrapper for the central Career Engine."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the ChatGPT Career Engine scanner")
    ...
    target_lane_reconciliation = reconcile_existing_non_target_jobs(
        _load_tracker(paths), bundle.get("taxonomy", {}), actor="chatgpt"
    )
    report = run_scan(Path(args.input), root=ROOT, scanner_id="chatgpt_scanner")
