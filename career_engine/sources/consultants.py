...
def _adapter_for_row(row: dict[str, Any], *, offline: bool):
    ...
    if provider:
        return ManagedCareerOpsAdapter(provider), f"managed_{provider}", "official_managed_ats"
    return build_adapter("jsonld", offline=offline), "jsonld", "official_page_jsonld"
...
def scan_consultants(*, root: Path, limit: int = 25, offline: bool = False) -> dict[str, Any]:
    ...
    for row in rows:
        if row.get("status") != "active" or row.get("scan") is not True:
            continue
        provider = _managed_provider(row)
        adapter_name = f"managed_{provider}" if provider else "jsonld"
        route = "official_managed_ats" if provider else "official_page_jsonld"
        source = {
            "source_id": row["id"],
        ...
        source["status"] = "empty"
        try:
            adapter, adapter_name, route = _adapter_for_row(row, offline=offline)
            source["adapter"] = adapter_name
            source["route"] = route
            company = _company_spec(row, provider)
            jobs = adapter.search(
                company=company,
            ...
            source["status"] = "ok" if jobs else "empty"
            for job in jobs:
                key = job.dedupe_key()
                if key in seen_jobs:
                    report["duplicates_dropped"] += 1
