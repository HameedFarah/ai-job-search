    ...
    for attempt in range(3):
        ...
        try:
            ...
            if exc.code == 401 and attempt < 2:
                # A long-running REGA pass can outlive the rclone access token
                # captured at startup. Refresh the existing authorized remote
                # once and retry the same unauthorized request; never expose the
                # token and keep the refreshed value for subsequent Sheet calls.
                try:
...
def ensure_queue_metadata(
    ...
    but creation is always chunked. Live enrichment should pass only the queue
    IDs it is about to write so a large legacy queue does not trigger thousands
    of metadata creates in one request.
    """
