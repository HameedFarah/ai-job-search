...
## Saudi/GCC Fork Configuration

This fork targets Saudi Arabia first and the wider GCC second. Portal enablement is controlled only through each portal skill's `enabled` frontmatter value.

The following Danish-market portal skills remain installed for upstream compatibility but must stay disabled during normal Career Engine runs:
...
The following GCC connector scaffolds must also stay disabled until their live interfaces, terms, parsing reliability and maintenance suitability are verified:

- `bayt-search`
- `naukrigulf-search`
- `paid-jobs-api-search`

Do not bypass these disabled states by invoking their CLIs directly during a normal scrape. A named diagnostic health check is allowed only when explicitly requested.
