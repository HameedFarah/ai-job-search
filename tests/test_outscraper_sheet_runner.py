...
class SheetRunnerTests(unittest.TestCase):
    ...
    def test_sheets_request_refreshes_rclone_token_once_on_401(self):
        unauthorized = urllib.error.HTTPError("https://example.test", 401, "Unauthorized", None, None)
        response = MagicMock()
        response.__enter__.return_value.read.return_value = b'{"ok":true}'
        with patch("runtime.outscraper_sheet_runner._REFRESHED_GOOGLE_TOKEN", ""), \
             patch("runtime.outscraper_sheet_runner.rclone_access_token", return_value="fresh-token") as refresh, \
             patch("runtime.outscraper_sheet_runner.urllib.request.urlopen", side_effect=[unauthorized, response]) as urlopen:
        ...
        self.assertEqual(first_request.get_header("Authorization"), "Bearer stale-token")
        self.assertEqual(second_request.get_header("Authorization"), "Bearer fresh-token")
