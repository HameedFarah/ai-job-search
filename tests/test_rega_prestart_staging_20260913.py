...
def test_prestart_stages_only_fully_verified_selected_route(monkeypatch, tmp_path):
    ...
    assert sender._stage_latest_verified_rega_records("token", []) == 1
    assert len(calls) == 1
...
def test_prestart_staging_dedupes_existing_email(monkeypatch, tmp_path):
    records = {
        "CE-1": {
            "master_id": "CE-1", "company": "Example", "identity_status": "confirmed", "domain": "example.sa",
    ...
    assert sender._stage_latest_verified_rega_records("token", [{"Email": "HR@example.sa"}]) == 0
