...
def main() -> int:
    ...
    try:
        ...
        print(
            f"note: {ref} not available (add the remote and fetch it first); nothing to triage.",
            file=sys.stderr,
        )
    ...
    lines: list[str] = []
    lines.append(f"Upstream `{ref}` has **{len(behind)}** commit(s) this fork lacks: **{len(review)}** worth reviewing, **{len(skip)}** probably skippable.")
    lines.append("")
